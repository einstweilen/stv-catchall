...
sender_bereinigen() {
    ...
    for (( i=0; i<=${#skip_name[@]}; i=i+4)); do
        ...
        # echo "${skip_name[i]}" "${skip_name[i+1]}" "${skip_name[i+2]}" "${skip_name[i+3]}"
    done
...
hilfetext() {
    echo "         Aufnahme aller Sendungen für alle SaveTV Sender programmieren"
    echo
    echo "In Zeile 8 und 9 den SaveTV Username und das Passwort eintragen oder"
    echo "beim manuellen Aufruf mit ./stvcatchall.sh username passwort übergeben"
    echo
    echo "Die Datei '$(basename "$send_skip")' anpassen, um einzelne Sender von der Programmierung"
    echo "auszunehmen"
    echo
    echo "--cleanup  -c  'Reste aufräumen' Funktion aufrufen"
    echo
    echo "--cleanupauto  'Reste aufräumen' ohne Sicherheitsabfrage ausführen, anschließend"
    echo "                wird die Catchall Channel Einrichtung durchgeführt."
    echo
    echo "HINWEIS:        Gelöschte Aufnahmen können nicht wiederhergestellt werden!"
    echo
